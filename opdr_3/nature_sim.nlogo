;; animals zijn de turtles (letterlijk) ik kan ze niet turtles noemen want dat is iets van netlogo zelf
...
;; pn staat voor power needed..hoeveel energie de turtles nodig hebben om te bewegen
animals-own [speed]
...
;; 3 meetwaardes
globals [survived death]

;; setup voor heel het programma. hier worden de turtles en birds gemaakt
to setup
  clear-all
  reset-ticks
  ...
  create-animals 50 [
    ...
    set shape "turtle"
    set color green
    ;; elke turtle heeft een random snelheid
...
;; simpele functie om de ask animals wat kleiner te maken
to move [loc]
  face loc
  fd speed
end
...
to go
  ...
  ask animals [
    ...
    ;; als een turtle moe wordt blijft het stilstaan en veranderd de kleur
    move ending
    ;; de turtle is save als deze bij de destination is dus verdwijnt ie (de die)
      if any? other destinations in-radius 1 [
        set speed 0
        set survived survived + 1
        die
      ]
      ;; als een bird in de radius is is de turtle dood
      if any? other birds in-radius 1 [
        set speed 0
        set death death + 1
        die
      ]
  ...
  ask birds [
    ;; dit kan je zien als de range van de vogel..als een turtle binnen deze range gaat, gaat de vogel op die turtle af
    ifelse any? other animals in-radius 3 [
      face one-of other animals in-radius 3 fd dive-speed
      ;; dit is de angst factor (of de dodge factor) van de turtle als een bird naar de turtles gaan nemen ze een stapje achteruit in de hoop dat de bird mist
      ask other animals [
        ;; je kan bij move niet een direction meegeven dus helaas moeten we het hier los invoeren
        face myself bk .2
      ]
    ]
    [
      ;; zo bewegen de birds in een random angle
